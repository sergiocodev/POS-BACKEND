...
public interface ActiveIngredientRepository extends JpaRepository<ActiveIngredient, Long> {
    ...
    @Query("SELECT a FROM ActiveIngredient a WHERE " +
           "(:name IS NULL OR LOWER(a.name) LIKE LOWER(CONCAT('%', :name, '%'))) AND " +
           "(:description IS NULL OR LOWER(a.description) LIKE LOWER(CONCAT('%', :description, '%')))")
    Page<ActiveIngredient> findAllPaged(
