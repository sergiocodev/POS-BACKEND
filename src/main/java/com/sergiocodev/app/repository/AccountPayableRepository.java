...
public interface AccountPayableRepository extends JpaRepository<AccountPayable, Long>, org.springframework.data.jpa.repository.JpaSpecificationExecutor<AccountPayable> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(a) FROM AccountPayable a WHERE a.status NOT IN :excludedStatuses AND a.dueDate >= CURRENT_DATE AND a.purchase.establishment.id = :establishmentId")
    Long getCountUpcomingDue(@org.springframework.data.repository.query.Param("excludedStatuses") List<AccountPayable.PayableStatus> excludedStatuses, @org.springframework.data.repository.query.Param("establishmentId") Long establishmentId);
    ...
    @org.springframework.data.jpa.repository.Query("SELECT COUNT(a) FROM AccountPayable a WHERE a.status NOT IN :excludedStatuses AND a.dueDate >= :start AND a.dueDate <= :end AND a.purchase.establishment.id = :establishmentId")
    Long getCountDueBetween(@org.springframework.data.repository.query.Param("start") java.time.LocalDate start, @org.springframework.data.repository.query.Param("end") java.time.LocalDate end, @org.springframework.data.repository.query.Param("excludedStatuses") List<AccountPayable.PayableStatus> excludedStatuses, @org.springframework.data.repository.query.Param("establishmentId") Long establishmentId);
